...
    }
   ...
   */
    ...
    function drawNotesForTrack(trackIndex, isActive, startTickVisible, endTickVisible, lowPitchVisible, highPitchVisible, pattern) {
        ...
        const originalTrackIndexArg = trackIndex;
        const isCurrentRawTrackGloballyActive = isActive; // True if 'trackIndex' is the global activeTrackIndex
        const patternContext = pattern; // The pattern providing context for drawing

        let notesToIterate;
        let isDrawingActiveSimplification = false;
        let contextPatternIdForNoteKey = null; // For s_patternId if drawing simplification
        let baseTrackIndexForRawNoteKey = -1;  // For trackIndex if drawing raw notes

        if (patternContext && patternContext.isSimplification && patternContext.s_notes && patternContext.id === activePatternId) {
            // Case 1: Drawing THE ACTIVE simplification pattern.
            // 'originalTrackIndexArg' filters which s_notes to draw based on their origin.
            ...
            notesToIterate = patternContext.s_notes.filter(s_note =>
                ...
                trackStates[s_note.originalTrackIndex]?.isVisible // And its original track must be visible
            );
            contextPatternIdForNoteKey = patternContext.id;
            // baseTrackIndexForRawNoteKey remains -1 (not used for this path's noteKey)
        } else {
            // Case 2: Drawing raw MIDI notes from rawTracksData.
            // This applies if patternContext is ROOT_ID, a non-simplification pattern,
            // or a simplification that isn't the *active* one (in which case we draw its underlying raw notes if applicable).
            ...
            if (!rawTrack || !rawTrack.notes || !trackStates[originalTrackIndexArg]?.isVisible) {
                return; // No raw notes for this track or it's hidden
            }
            notesToIterate = rawTrack.notes;
            baseTrackIndexForRawNoteKey = originalTrackIndexArg;
            // contextPatternIdForNoteKey remains null
        }

        if (!notesToIterate || notesToIterate.length === 0) {
            return; // No notes to draw after filtering
        }


        notesToIterate.forEach((note, noteIndex) => {
            let styleContextTrackIndex; // NEW - for isHarmonyTrack, isDrumTrack checks
            ...
            let noteKey = JSON.stringify({ trackIndex, noteIndex });
            let isSelected = selectedNotes.has(noteKey) && !abcState.isPlaying;
            let isInScale = isNoteInScale(note.pitch); // Check scale membership 

            if (isDrawingActiveSimplification) {
                const s_noteIndex = patternContext.s_notes.indexOf(note); // Get true index in original s_notes
                if (s_noteIndex === -1) { console.warn("Simplification note not found in its source array during key generation.", note); return; }
                noteKey = JSON.stringify({ s_patternId: contextPatternIdForNoteKey, s_noteIndex: s_noteIndex });
                styleContextTrackIndex = note.originalTrackIndex; // Use the note's stored original track
            } else {
                const rawNoteIndex = rawTracksData[baseTrackIndexForRawNoteKey].notes.indexOf(note);
                if (rawNoteIndex === -1) { console.warn("Raw note not found in its source array during key generation.", note); return; }
                noteKey = JSON.stringify({ trackIndex: baseTrackIndexForRawNoteKey, noteIndex: rawNoteIndex });
                styleContextTrackIndex = baseTrackIndexForRawNoteKey;
            }
            ...
            if (!isDrawingActiveSimplification && patternContext.id !== ROOT_ID && !patternContext.isSimplification) {
                if (!noteInPattern(note, patternContext, baseTrackIndexForRawNoteKey)) return;
            }
            ...
            // --------------------------------------------------------- 
            const isNoteEffectivelyActiveForStyle = isDrawingActiveSimplification ?
                (trackStates[note.originalTrackIndex]?.isVisible) : // Active if from active simp & orig track visible
                isCurrentRawTrackGloballyActive;  // Use global active status for raw notes


            if (isNoteEffectivelyActiveForStyle && !isSelected) {                 // don’t override selections 
            ...
            if (!isSelected) { // Non-selected notes 
                if (isNoteEffectivelyActiveForStyle) { // Active track - use velocity 
